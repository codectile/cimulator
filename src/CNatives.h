...
public:
	...
	int objectid;
	btRigidBody* col;
	colObject(int idx, int objid, int mdlid, btRigidBody* body) : index(idx), objectid(objid), modelid(mdlid), col(body)
	{
	}
	~colObject()
	{
...
colObject* rigidBody[CRMAX_BODY];
...
static int index_availability[CRMAX_BODY];
